...
*/
:- module(attributes, [
   'attribute'/1,get_atts/2,put_atts/2,
   set_dict_atts_reader/1,
   dict_to_attvar/2,dict_to_attvar/3,
...
atts_exist(_A,_At):- current_prolog_flag(dict_atts_reader,true),!.
...
put_atts(Var,M,List):- prolog_flag(attvar_pn,true),!,put_atts(+,Var,M,List).
...
put_atts(PN,Var,M,At):-var(At),!,throw(error(instantiation_error, M:put_atts(Var,PN:At))).
%put_atts(PN,Var,user,Atts):-!, put_atts(PN,Var,tst,Atts).
put_atts(PN,Var,M, X+Y):-!, put_atts(PN,Var,M, X),put_atts(PN,Var,M,+Y).
put_atts(PN,Var,M, X-Y):-!, put_atts(PN,Var,M, X),put_atts(PN,Var,M,-Y).
put_atts(PN,Var,M, +X+Y):-!, put_atts(PN,Var,M, +X),put_atts(PN,Var,M,+Y).
put_atts(PN,Var,M, +X-Y):-!, put_atts(PN,Var,M, +X),put_atts(PN,Var,M,-Y).
put_atts(PN,Var,M, List):- is_list(List),!,atts_module(Var,M),maplist(put_atts(PN,Var,M),List).
put_atts(_, Var,M,  +At):-!, put_atts(+,Var,M,At).
put_atts(PN,Var,M,  -At):- invert_pn(PN,NP),!,put_atts(NP,Var,M,At).
%put_atts(PN,Var,_,(M:At)):- \+ meta_handler_name(M), !,put_atts(PN,Var,M,At).
%put_atts(PN,Var,M, Meta):- \+ \+ clause(M:meta_hook(Meta,_,_),_), !, forall(M:meta_hook(Meta,P,A),put_atts(PN,Var,M,P=A)).
% =(+a,b) -->   +(A=B).
put_atts(PN,Var,M, Pair):- compound(Pair),Pair=..[P,Arg1,Arg2],attsep(P),compound(Arg1),call((Arg1=..List,append(Head,[Last],List),At=..[P,Last,Arg2],append(Head,[At],ListNew),Try=..ListNew,!,put_atts(PN,Var,M, Try))).
% put_atts(PN,Var,_, Hook):-  handler_fbs(+ Hook,Number), Number>0, !,PNHook=..[PN,Hook], put_datts(Var, PNHook).

put_atts(PN,Var,M,Pair):- !,
  atts_to_att(Pair,Tmpl),
...
set_dict_atts_reader(X):- set_prolog_flag(dict_atts_reader,X).
...
term_expansion(Dict,X):- current_prolog_flag(dict_atts_reader,true),'$current_source_module'(M),dict_to_attvar(M,Dict,X).

:- multifile(system:goal_expansion/2).
:- dynamic(system:goal_expansion/2).
:- module_transparent(system:goal_expansion/2).
system:goal_expansion(Dict,X):- current_prolog_flag(dict_atts_reader,true),'$current_source_module'(M),dict_to_attvar(M,Dict,X).
